...
export default function Contact1stSection() {
    ...
    async function sendMessage(e) {
        ...
        checkInput(inputsValue.company, 'companyInput')
        ...
        if (response.status === 200) {
            setInputValue({...inputsValue, name: '', email: '', company: '', subject: '', message: ''})
            setInputError({...inputError, nameErr: "", emailErr: "", subjectErr: "", messageErr: ""})
            setFormState({...formState, sending: false, submitted: true, success: true})
            setInputFocus({...inputsFocus, nameInput: false, emailInput: false, companyInput: false, subjectInput: false, messageInput: false})
            window.alert('Message sent successfully ✅✅✅')
        } else {
            setFormState({...formState, sending: false, submitted: true, success: false})
                        ...
                        <form onSubmit={(e) => {e.preventDefault(); sendMessage(e)}}>
                            <div className="input-block-wrapper">
                                <div className="input-wrapper">
                                    ...
                                    <input value={inputsValue.name} onKeyDown={() => {setInputError({...inputError, nameErr: null})}} onChange={(e) => setInputValue({...inputsValue, name: e.target.value})} onBlur={() => {checkInput(inputsValue.name, 'nameInput')}} onFocus={() => {setInputFocus({...inputsFocus, nameInput: true})}} type="text" className="nameInput" />
                                    <span>{inputError.nameErr}</span>
                                </div>
                                <div className="input-wrapper">
                                    <p style={{transform: inputsFocus.emailInput && 'translateY(8px)'}}>Email</p>
                                    <input value={inputsValue.email} onKeyDown={() => {setInputError({...inputError, emailErr: null})}} onChange={(e) => setInputValue({...inputsValue, email: e.target.value})} onBlur={() => {checkInput(inputsValue.email, 'emailInput')}} onFocus={() => {setInputFocus({...inputsFocus, emailInput: true})}} className="emailInput" type="text" />
                            ...
                            <div className="input-block-wrapper">
                                <div className="input-wrapper">
                                    ...
                                    <input value={inputsValue.company} onChange={(e) => setInputValue({...inputsValue, company: e.target.value})} onBlur={() => {checkInput(inputsValue.company, 'companyInput')}} onFocus={() => {setInputFocus({...inputsFocus, companyInput: true})}} type="text" className="companyInput" />
                                    <span id="company-span">Optional</span>
                                </div>
                                <div className="input-wrapper">
                                    <p style={{transform: inputsFocus.subjectInput && 'translateY(8px)'}}>Subject</p>
                                    <input value={inputsValue.subject} onKeyDown={() => {setInputError({...inputError, subjectErr: null})}} onChange={(e) => setInputValue({...inputsValue, subject: e.target.value})} onBlur={() => {checkInput(inputsValue.subject, 'subjectInput')}} onFocus={() => {setInputFocus({...inputsFocus, subjectInput: true})}} type="text" className="subjectInput"/>
                            ...
                            <div className="input-block-wrapper">
                                <div className="input-wrapper">
                                    ...
                                    <textarea value={inputsValue.message} onKeyDown={() => {setInputError({...inputError, messageErr: null})}} rows="7" onChange={(e) => setInputValue({...inputsValue, message: e.target.value})} onBlur={() => {checkInput(inputsValue.message, 'messageInput')}} onFocus={() => {setInputFocus({...inputsFocus, messageInput: true})}} type="text" className="messageInput" />
                                    <span>{inputError.messageErr}</span>
                                </div>
                            </div>
                            <div className="send-btn-wrapper">
                                <button onMouseDown={() => {setInputValue({...inputsValue, company: formatCompany(inputsValue.company)}); setInputFocus({...inputsFocus, companyInput: true})}} type="submit">{formState.sending ? <Icon style={{color: '#fff', margin: 'auto'}} width='20' icon="eos-icons:loading" /> : 'Send Message'}</button>
                            </div>
